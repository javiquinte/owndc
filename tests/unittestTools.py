"""Define a Runner and a Test Result capable of pretty print results."""
import sys
import unittest


class WITestRunner(object):
    """Run the tests and pretty print the results."""

    def __init__(self, outStream=sys.stderr, mode=1):
        """WITestRunner constructor.
...
class WITestResult(unittest.TestResult):
    ...
    def addSuccess(self, test):
        """Method executed when a test is passed."""
        unittest.TestResult.addSuccess(self, test)
        self.testRunner.write('[' + self.OKGREEN + 'OK' + self.ENDC + ']\n')

    def addError(self, test, err):
        """Method executed when an error is found while running a test."""
        unittest.TestResult.addError(self, test, err)
        self.testRunner.write('[' + self.WARNING + 'ERROR' + self.ENDC + ']\n')

    def addFailure(self, test, err):
        """Method executed when a failure is informed while running a test."""
    ...
    def printErrorList(self, errorType, errors):
        """Print error messages from the tests."""
